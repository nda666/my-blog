...
> = ({ parentsData }) => {
  return {
    title: `Category - ${parentsData["routes/admin/__page"].env.APP_NAME}`,
...
export default function Category() {
  ...
  const columns = useMemo<ColumnDef<ICategory, unknown>[]>(
    () => [
      {
        header: "Action",
        footer: "Action",
  ...
  const dataQuery = useQuery(
    ["category.table", fetchDataOptions],
    async () => {
  ...
  const deleteOnePost = async (id: string) => {
    const deleted = await fetch(`category/${id}/delete`, { method: "POST" });
